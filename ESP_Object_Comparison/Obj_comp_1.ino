...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (command == "CAPTURE") {
      ...
      //esp_camera_deinit();     // Turn off camera
      //esp_camera_init(&config); // Re-init camera
    ...
    // Show result like <Missing:person(-1)>
...
void displayToLCD(String msg) {
  ...
  int separator = msg.indexOf('|');  // separator between lines

  if (separator != -1) {
    String line1 = msg.substring(0, separator);     // Missing or No missing objects
    String line2 = msg.substring(separator + 1);    // Detected object summary
    ...
    lcd.setCursor(0, 0);
    lcd.print(msg);  // fallback in case '|' is missing
  }
}
