...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (command == "CAPTURE") {
      ...
      //lcd.setCursor(0, 1);
      //lcd.print("Image Sent");
    }

    // Handle framed object detection result (like <person:2,clock:1>)
    else if (command.startsWith("<") && command.endsWith(">")) {
      String clean = command.substring(1, command.length() - 1);  // Remove < and >
      displayToLCD(clean);  // Show detected labels
    }
  }
}
...
void displayToLCD(String msg) {
  ...
  if (separator != -1) {
    lcd.setCursor(0, 0);
    lcd.print(msg.substring(0, separator));
    lcd.setCursor(0, 1);
    lcd.print(msg.substring(separator + 1));
  } else {
    lcd.setCursor(0, 0);
    lcd.print(msg);
  }
